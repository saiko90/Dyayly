...
export default function AdminDashboard() {
  ...
  const updateOrderStatus = async (id: string, status: string, label: string) => {
    setUpdatingOrder(id);
    try {
      const res = await fetch('/api/admin/orders', {
        method:  'PATCH',
              ...
              {loadingOrders ? (
                ...
                <div className="space-y-4">
                  {orders.map(order => {
                    ...
                    const isBusy  = updatingOrder === order.id;
                      ...
                      >
                        ...
                        {order.status === 'paid' && (
                          <div className="px-6 py-4 bg-green-50 border-t border-green-100 flex flex-wrap gap-3">
                            ...
                            <button
                              onClick={() => updateOrderStatus(order.id, 'shipped', `📦 Commande #${ref} marquée comme expédiée !`)}
                              disabled={isBusy}
                            ...
                            >
                              ...
                              {isBusy ? 'En cours…' : 'Marquer comme Expédié 📦'}
                            </button>
                            <button
                              onClick={() => {
                                if (window.confirm(`Annuler la commande #${ref} ? Cette action est définitive.`)) {
                                  updateOrderStatus(order.id, 'cancelled', `Commande #${ref} annulée.`);
                                }
                              }}
                              disabled={isBusy}
